...
import { exportUrl, fetchTask, PLATFORM_META, submitFeedback, zipUrl, type PlatformListing, type TaskDetail } from "@/lib/api";
...
function stageToIndex(task: TaskDetail): number {
  ...
  const s = task.stage || "";
  if (s.includes("合规") || s.includes("自愈") || s.includes("修订")) return 4;
  if (s.includes("生成")) return 2;
  if (s.includes("规则")) return 1;
  return 0;
}
...
export default function ResultPage() {
  ...
  const poll = useCallback(async () => {
    ...
    try {
      ...
      setTask(detail);
      if (detail.status === "running" || detail.status === "queued") setTimeout(poll, 1500);
    } catch {
      ...
      <main className="pb-24">
        ...
        <section className="contour-bg relative overflow-hidden">
          <div className="mx-auto max-w-3xl px-6 py-16">
            ...
            <div className="mt-12">
              {STEPS.map((step, i) => {
                const isDone = i < idx || (idx === 4 && i < 4);
                const isActive = i === idx || (idx === 2 && (i === 2 || i === 3));
                return (
  ...
  return (
    <main className="pb-24">
      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        {current && (
          <div className="mt-10 grid gap-6 lg:grid-cols-[3fr_2fr] animate-fade-up">
            {/* 左：文案内容 */}
            <section className="card p-7">
              <div>
                <div className="flex items-baseline justify-between gap-3">
                  ...
                  <span className="font-mono text-[11px] tabular-nums text-ink-400">
                    {current.title.length}/{TITLE_LIMITS[current.platform] || 200}
                  </span>
                </div>
                <p className="mt-3 text-base font-medium leading-7 text-ink-900">{current.title || "—"}</p>
              </div>
              ...
              {current.bullets.length > 0 && (
                <div className="mt-6 border-t border-ink-100 pt-6">
                  <div className="flex items-baseline justify-between gap-3">
                    ...
                    <span className="font-mono text-[11px] tabular-nums text-ink-400">
                      {current.bullets.length} 条
                    </span>
                  </div>
                  <ul className="mt-3 space-y-2">
                    {current.bullets.map((b, i) => (
                      <li key={i} className="flex gap-2.5 text-sm leading-6 text-ink-600">
                        <span className="mt-0.5 font-mono text-[10px] font-medium tabular-nums text-brand-400">
            ...
            <aside className="space-y-6">
              <PublishPanel
                ...
                compliancePassed={current.compliance_passed}
              />
              ...
              <div className="card p-6">
                <div className="flex items-center justify-between">
                  <h2 className="eyebrow">合规报告</h2>
                  <span
                    className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${
                      current.compliance_passed ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"
                    }`}
                  >
                    {current.compliance_passed ? "✓ 通过" : "存在风险"}
                  </span>
                </div>
                ...
                <ul className="mt-4 space-y-2">
                  ...
                  {current.compliance.map((issue, i) => (
                    <li
                      key={i}
